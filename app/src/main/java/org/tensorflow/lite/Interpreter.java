...
//  public void run(/*@NonNull*/ Object input, /*@NonNull*/ float[] dst) {
//	    Object[] inputs = {input};
//	    
//	    runForMultipleInputsOutputs(inputs, FloatBuffer.wrap(dst));
//	  }
  ...
  public void runForMultipleInputsOutputs(
	      /*@NonNull*/ Object[] inputs, /*@NonNull*/ FloatBuffer dst) {
	    if (wrapper == null) {
	      throw new IllegalStateException("Internal error: The Interpreter has already been closed.");
	    }
	    Tensor[] tensors = wrapper.run(inputs);
	    if (dst == null || tensors == null /*|| dst.size() > tensors.length*/) {
	      throw new IllegalArgumentException("Output error: Outputs do not match with model outputs.");
	    }
	    final int size = tensors.length;
	   
	     tensors[0].copyTo(dst);
	  }
